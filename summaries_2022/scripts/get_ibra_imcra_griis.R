...
# Changes in invasive species by IBRA & IMCRA region (from 1900, 5-year interval) 
# ibra_griis_5year and imcra_griis_5year: 
# ibra or imcra, yearStart, yearEnd, griisStatus, recordCount, speciesCount
...
# |- get counts ------
grouped_imcra <- joined_imcra |> 
  group_by(imcraRegion, period_key, griisStatus) |> 
  summarise(recordCount = sum(count),
            speciesCount = n_distinct(speciesId))

# |- convert time periods into start and end year -----
imcra_griis_5year <- grouped_imcra |> 
  mutate(
    yearStart = case_when(
      period_key == 1 ~ 1900,
      period_key == 2 ~ 1971,
